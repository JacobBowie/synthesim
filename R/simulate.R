...
#' @param variant Which RHS form to use. Default `"quadratic"` matches the
#'   original Vensim/PySD oracle. The `"linear"` variant won model
#'   selection on real RT data — pass `variant = "linear"` explicitly
#'   when forward-simulating the linear form. Default is NOT flipped to
#'   "linear" because doing so would silently break the Vensim-oracle
#'   path that gates the R port's bit-exactness against PySD.
